...
export const metadata: Metadata = {
  title: "Pika 料金プラン徹底解説【2026年最新】Free/Standard/Pro/Unlimited",
  description: "AI動画生成PikaのFree・Standard(月10ドル)・Pro(月35ドル)・Unlimitedの料金プランを徹底解説。Pikaffectsの使い方も。",
  keywords: ["Pika 料金", "Pika AI", "AI 動画生成"],
  alternates: { canonical: `${siteConfig.url}/guide/pika-pricing` },
};
export default function Page() {
  const jsonLd = { "@context": "https://schema.org", "@type": "Article", headline: "Pika 料金プラン徹底解説", description: "Free/Standard/Pro/Unlimitedを解説", author: { "@type": "Organization", name: siteConfig.name }, publisher: { "@type": "Organization", name: siteConfig.name }, datePublished: "2026-04-11", dateModified: "2026-04-11" };
  return (
    <>
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(jsonLd) }} />
      <div className="max-w-3xl mx-auto px-4 py-8">
        <nav className="flex items-center gap-2 text-sm text-muted mb-8"><Link href="/" className="hover:text-primary transition-colors">ホーム</Link><span>/</span><Link href="/guide" className="hover:text-primary transition-colors">ガイド</Link><span>/</span><span className="text-foreground">Pika料金</span></nav>
        ...
        <h1 className="text-3xl md:text-4xl font-bold mb-6 leading-tight">Pika 料金プラン徹底解説【2026年最新】</h1>
        <p className="text-muted text-lg mb-10 leading-relaxed">SNS動画に強いAI動画生成Pikaの全プランを解説。Pikaffectsの活用法も。</p>
        <section className="mb-12">
          <h2 className="text-2xl font-bold mb-6">Pika料金プラン一覧</h2>
          <div className="space-y-4">
            {[
              { name: "Free", price: "無料", desc: "月250クレジット。ウォーターマーク付き。お試し用。" },
              { name: "Standard", price: "月10ドル（約1,500円）", desc: "月700クレジット。ウォーターマークなし。個人クリエイター向け。" },
              { name: "Pro", price: "月35ドル（約5,250円）", desc: "月2,000クレジット。優先生成。商用利用本格OK。" },
              { name: "Unlimited", price: "月60ドル（約9,000円）", desc: "無制限生成。最優先。プロクリエイター向け。" },
            ].map((p) => (
